...
# payment proof
GetPaymentProofAPI = "https://api-stage.wallter.com/api/v2/payments/pop/"
...
def main_flow():
    ...
    with requests.Session() as s:
        ...
        try:
            ...
            #incremental_Id = decode_txn_id(paymentID)[0]
        ...
        sleep(15)
        try:
            GetPaymentProofResponse = s.get(GetPaymentProofAPI + str(paymentID), headers={'Authorization': "Bearer " + loginToken}, verify=False)

            if GetPaymentProofResponse.status_code in (200, 201, 202, 204):
                logger.info(paymentID)
                logger.info("Proof of payment is accepted")

            else:
                logger.info(GetPaymentProofResponse.status_code)
        except requests.exceptions.RequestException as e:
            logger.exception(e)
